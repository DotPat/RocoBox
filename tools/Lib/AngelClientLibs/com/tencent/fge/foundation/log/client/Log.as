...
class SOLog
{
   ...
   public static function ready(param1:Object) : void
   {
      ...
      {
         name = String(getLSOName(param["name"]));
      }
      else
      {
         name = String(getLSOName(null));
      }
      try
      {
      ...
      if(ms_so)
      {
         ...
         date = new Date();
         time = String(getTimeString(date));
         keepDate = 0;
         if(param.hasOwnProperty("keepDate"))
         {
            ...
            {
               date.date -= keepDate;
               timeOld = String(getTimeString(date));
